...
File information
=====
    first edit date   : 2020/10/28
    first editer      : choi keonghun
    last edit date    : 2020/10/28
...
def read_img(file_dir: str, color_type: int, resize: list = None, is_last_channel: bool = True, is_debug: bool = False):
    """ Image file to data  
    Args:  
        data_list :
        is_last_axis :
...
def make_video(save_dir: str, imgs: ["np.uint8 ndarray"], ext: ["Defalt ext is avi"] = None, frame: float = 30.0, shape: [int, int] = None) -> None:
...
def img_render(img: ["np.uint8 ndarray"], window_name: str, is_zero2one: bool = True, is_First_channel: bool = False, input_text: str = None, save_dir: str = None, is_DEBUG: bool = False) -> None:
    """
    this code must be use 3ch image
    """
    _img = img
    if is_zero2one:
        _img *= PIXEL_MAX
    ...
    if input_text is not None:
        [_text_w, _text_h], _ = cv2.getTextSize(text=input_text, fontFace=cv2.FONT_HERSHEY_DUPLEX, fontScale=1, thickness=2)
        _h, _w, _c = np.shape(_img)
        _text_shell = 255 * np.ones((_text_h + (2 * IMAGE_TEXT_PADDING), _text_w + (2 * IMAGE_TEXT_PADDING), _c), dtype=np.uint8)
...
def Neighbor_Confusion_Matrix(img: ["np.uint8 ndarray"], target: ["np.uint8 ndarray"], interest: ["np.uint8 ndarray"]) -> ["Confusion Matrix"]:
    ...
    return (_nb_tp.astype(np.uint8), _nb_tn.astype(np.uint8), _nb_fn.astype(np.uint8), _nb_fp.astype(np.uint8))


def Calculate_Confusion_Matrix(image: ["np.uint8 ndarray"], target: ["np.uint8 ndarray"], interest: ["np.uint8 ndarray"] = None) -> ["Confusion Matrix"]:
    if interest is None:
...
def img_resize(img: ["np.uint8 ndarray"], size: ["[int, int] or [float, float]"]):
    """
    """
    try:
        h, w, _ = np.shape(img)
...
def make_canny(img: ["np.uint8 ndarray"], high: int, low: int, is_First_ch=True, is_zero2one: bool = True) -> ["np.uint8 ndarray"]:
    if is_First_ch:
        if np.shape(img)[0] == 3:
    ...
    cany_img = np.array([cany_img, cany_img, cany_img], np.uint8) if is_First_ch else np.dstack([cany_img, cany_img, cany_img])
    cany_img = cany_img / 255 if is_zero2one else cany_img
    return cany_img


def make_noise_in_image(img: ["np.ndarray"], noise_rate: float, noise_type: ["noise type"] = SALT_N_PAPPER, noise_parameter: ["each noise parameter"] = None) -> ["noise image"]:
    try:
    ...
    if noise_type == SALT_N_PAPPER:
        ...
        if _c:
            for _ct_c in range(_c):
                ...
                noise_img[:, :, _ct_c] = np.where(_noise_mask == 1, img[:, :, _ct_c], _img_max - img[:, :, _ct_c])
        else:
            _img_max = np.max(img)
